...
<tr class="shipping <?php echo ( ! empty( $class ) ) ? $class : ''; ?>" data-order_item_id="<?php echo $item_id; ?>">
	...
	<td class="line_cost" width="1%">
		...
		<?php if ( 'seller' === dokan()->commission->get_shipping_fee_recipient( $order->get_id() ) && isset( $item['cost'] ) && wc_round_tax_total( $item['cost'] ) > $refunded ) : ?>
	...
	if ( empty( $legacy_order ) && wc_tax_enabled() ) :
		...
		if ( method_exists( dokan()->commission, 'get_shipping_tax_fee_recipient' ) ) {
			$shipping_tax_fee_recipient = dokan()->commission->get_shipping_tax_fee_recipient( $order );
		} else {
			$shipping_tax_fee_recipient = dokan()->commission->get_tax_fee_recipient( $order->get_id() );
		}
